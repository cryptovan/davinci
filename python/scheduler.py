...
def scheduler(t=60,n=3,vol_in_btc=2):
	...
	dataLabels = ['date','volume','openBuyOrders','openSellOrders','lastPrice']
		...
		# while (nn<10):
			for market in marketsList[:x]: #traverse the markets 
				...
				try:
					...
					newdata = pd.DataFrame([{'date':new_date,'volume':new_volume,'openBuyOrders':new_buy_orders,'openSellOrders':new_sell_orders,'lastPrice':new_last_price}]) # append new data to df
					...
					if len(volumes)>=n: 
						av = average(volumes[-n-2:],n)
						vol_ratio = new_volume/av
						...
						if (((vol_ratio >= volume_alert_ratio) or (volume_alert_ratio <= (1-vol_ratio)))):
							...
							print(">>> Alert! {} Pair {} had a change of volume of {:.1f} percent in the last {:.2f} seconds".format(new_date, market, vol_percentage,t))
							
							# add alert data to log
							new_data = {'date':new_date,'market':market,'vol_percentage':vol_percentage,'timeframe':t}
							vol_datalabels = new_data.keys()
							...
							new_vol_data = pd.DataFrame([new_data]) # append new data to df
					...
					# volumes = df.tail(n)['volume'].tolist()
					if len(volumes)>=n: 
						vol_prev = volumes[-2:][0]
						vol_change_in_btc = (new_volume-vol_prev)*new_last_price
						...
						if (vol_change_in_btc >= vol_in_btc):
							...
							# add alert data to log
							new_data = {'date':new_date,'market':market,'btc_delta':vol_change_in_btc,'timeframe':t}
